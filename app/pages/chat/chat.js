...
//Directives
.directive('chatNav', function() {
  return {
    templateUrl: 'pages/chat/partials/nav.html'
  };
})

.directive('chatModals', function() {
  return {
...
//Controllers
.controller('chatController', ['$scope', '$timeout', '$pusher', 'Upload', '$routeParams', function($scope, $timeout, $pusher, Upload, $routeParams) {
  var chatCtrl = this;
  ...
  function bindEvents() {
    chatCtrl.pusher.connection.bind('connected', function() {
      ...
      chatCtrl.channel.bind('pusher:subscription_error', function(members) {
        $('#alertError').text("Hubo un error al unirse a la sala, posiblemente la contraseña sea incorrecta");
  ...
  function uploadFiles(file, errFiles) {
    ...
    if($scope.errFile){
      $('#alertError').text("Hubo un error al enviar el archivo, asegurate de que sea una foto o video y que pese menos de 10MB");
      $('#alertModal').modal('show');
      return;
    }
    if (file) {
